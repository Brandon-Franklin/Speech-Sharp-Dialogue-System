...
namespace XNodeEditor {
    public static class NodeEditorPreferences {
        ...
        public class Settings : ISerializationCallbackReceiver {
            ...
            public bool autoSaveNodes = true;
            public bool autoUpdateDialogueClassesOnSave = true;
            public bool zoomToMouse = true;
            public bool portTooltips = true;
            [SerializeField] private string typeColorsData = "";
            [NonSerialized] public Dictionary<string, Color> typeColors = new Dictionary<string, Color>();

            public static Color32 chatConnectionColor = new Color32(255, 255, 255, 255);
            public static Color32 branchConnectionColor = new Color32(255, 255, 255, 255);
            public static Color32 eventConnectionColor = new Color32(255, 255, 255, 255);
        ...
        private static void SystemSettingsGUI(string key, Settings settings) {
            ...
            settings.autoSaveNodes = EditorGUILayout.Toggle(new GUIContent("Autosave Graphs", "Saves the graph after every change to ensure no progress is lost. Disable for better editor performance"), settings.autoSaveNodes);
            settings.autoUpdateDialogueClassesOnSave = EditorGUILayout.Toggle(new GUIContent("Autosave Scripts", "Updates generated character scripts when you save your project. Disable for better editor performance"), settings.autoUpdateDialogueClassesOnSave);
        ...
        private static void TypeColorsGUI(string key, Settings settings) {
            //Label
            EditorGUILayout.LabelField("Tether Color", EditorStyles.boldLabel);

            //show explicit colors
            //Settings.chatConnectionColor = EditorGUILayout.ColorField("Chat Node", Settings.chatConnectionColor);
            //Settings.branchConnectionColor = EditorGUILayout.ColorField("Branch Node", Settings.branchConnectionColor);
            //Settings.eventConnectionColor = EditorGUILayout.ColorField("Event Node", Settings.eventConnectionColor);
        ...
        /// <summary> Return color based on type </summary>
        public static Color GetTypeColor_Explicit(System.Type type)
        {
            VerifyLoaded();
            if (type == null) return Color.gray;
            Color col;

            if (type == typeof(Dialogue.Chat))
            {
                return Settings.chatConnectionColor;
            }
            else if (type == typeof(Dialogue.Branch))
            {
                return Settings.branchConnectionColor;
            }
            else if (type == typeof(Dialogue.Event))
            {
                return Settings.eventConnectionColor;
            }
            else
            {
                return Color.gray;
